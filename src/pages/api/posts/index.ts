...
export const GET: APIRoute = async (context) => {
  try {
    ...
    const result = await auth.db.prepare(
      `SELECT ${MANAGED_POST_COLUMNS} FROM posts p LEFT JOIN users u ON u.id = p.author_id
       ${owner ? '' : 'WHERE p.author_id = ?'} ORDER BY p.updated_at DESC`,
    );
    const rows = owner
      ? await result.all<ManagedPostRow>()
      : await result.bind(auth.session.user.id).all<ManagedPostRow>();
    return json({ posts: rows.results.map(serializeManagedPost) });
  } catch {
...
export const POST: APIRoute = async (context) => {
  try {
    ...
    if (!canSetPostStatus(auth.session.user, { authorId: auth.session.user.id }, post.status)) {
    ...
    await auth.db.prepare(
      ...
      id, post.slug, post.title, post.description, post.body, post.pubDate, now,
    ...
    return json({ id, post: { id, ...post, content: post.body, authorId: auth.session.user.id, createdAt: now, updatedAt: now } }, 201);
  } catch (error) {
